...
export default function EditRecipeScreen() {
  ...
  const { data, loading: recipeLoading } = useRecipe({
    variables: { id: id! },
    skip: !id,
  });
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      servesCount: recipe.servesCount ?? 1,
    });
    setIngredients(recipeIngredientsToDrafts(recipe.ingredients));
    setSteps(recipe.steps?.length ? recipe.steps : ['']);
    setExistingImageUri(recipe.imageKey);
    setInitialized(true);
  }, [recipe, initialized]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: updateData } = await updateRecipe({
        variables: {
          id: recipe.id,
          input: {
            ...
            servesCount: form.servesCount > 0 ? form.servesCount : null,
            ...(imageKey ? { imageKey } : {}),
          },
        },
        refetchQueries: ['GetRecipes', 'GetRecipe'],
      });

      if (updateData?.updateRecipe) router.back();
    } catch (uploadError) {
      setImageError(
